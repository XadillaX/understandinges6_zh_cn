...
## 广知符号 (Well-Known Symbols)

我们除了可以自己定义符号以外，系统还预定义了很多符号（即广知符号）。这些符号在 JavaScript 中表示预内处理操作的常见行为。每一个广知符号都被表示成 `Symbol` 中的一个属性，例如 `Symbol.create` 对应的是 `@@create` 符号。

ECMAScript 5 和 ECMAScript 6 的一个中心主题就是在 JavaScript 中去暴露和定义一些“魔术”组成——一些无法被开发者所模拟的部分。ECMAScript 6 通过让语言暴露更多的预内处理逻辑来遵循这样的传统。它主要是通过使用符号原型属性来定义某个对象的基本行为。

> 重写一个广知符号的方法会将一个传统对象调教成为一个异端对象，因为这改变了一些内联的默认行为。
